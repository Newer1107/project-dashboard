...
export default function AnalyticsDashboard() {
  ...
  const filteredProjects = useMemo(() => {
    ...
    const sourceData = activeTab === "rbl" ? rblGroups : beProjects;

    return sourceData.filter((p) => {
      switch (activeFilter.type) {
        case "domain":
          return p.domain === activeFilter.value;
        case "sdg":
          return String(p.sdg) === activeFilter.value;
        case "class":
          return p.groupId.startsWith(
            activeFilter.value.replace("BE-", "").replace("TE-", ""),
          );
        case "category":
          return p.category === activeFilter.value;
        case "application":
          return p.projectApplication === activeFilter.value;
        case "techFocus":
          // Re-run the keyword matching for this specific project
          ...
          let matchedCategory = "Platforms & Web Apps"; // Default fallback
          ...
          return matchedCategory === activeFilter.value;
        default:
          return true;
      }
    });
      ...
      <div className="relative w-full max-w-7xl mx-auto px-4 md:px-6 pb-12 pt-32 space-y-8 text-neutral-800 dark:text-neutral-100">
                    ...
                    >
                      {metrics.domainData.map((d, i) => (
                        <Cell
                          ...
                          // Optional: Dim unselected slices
                          opacity={
                            activeFilter && activeFilter.value !== d.name
                              ? 0.3
                              : 1
                          }
                        />
                  ...
                  >
                    {metrics.sdgData.map((d, i) => (
                      <Cell
                        key={i}
                        fill={DONUT_COLORS[i % DONUT_COLORS.length]}
                        // Dims the unselected bars
                        opacity={
                          activeFilter && activeFilter.value !== String(d.sdg)
                            ? 0.3
                            : 1
                        }
                      />
                ...
                >
                  {metrics.techFocusData.map((d, i) => (
                    <Cell
                      ...
                      // Dims the unselected bars
                      opacity={
                        activeFilter && activeFilter.value !== d.name ? 0.3 : 1
                      }
                    />
                  ))}
                </Bar>
              ...
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={metrics.applicationData} barSize={25}>
                  ...
                  <Bar
                    ...
                    className="cursor-pointer"
                  >
                    {metrics.applicationData.map((_, i) => (
                      <Cell
                        key={i}
                        fill={DONUT_COLORS[i % DONUT_COLORS.length]}
                      />
        ...
        {activeFilter && (
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
              <div>
                <h3 className="text-xl font-bold text-neutral-900 dark:text-white flex items-center gap-2">
                  Projects for:{" "}
                  <span className="text-indigo-500">{activeFilter.value}</span>
                </h3>
                <p className="text-sm text-neutral-500 mt-1">
                  Found {filteredProjects.length} matching project
            ...
            {/* 2. The Grid of Projects */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredProjects.map((proj, idx) => (
                <div
                  key={idx}
                  className="p-4 rounded-xl border border-neutral-200 dark:border-neutral-800 bg-white/50 dark:bg-neutral-950/50 hover:bg-white dark:hover:bg-neutral-900 transition-colors"
                >
