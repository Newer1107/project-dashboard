  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      const firstFocusable =
        filterPopoverRef.current?.querySelector<HTMLElement>(
                ...
                <AnimatePresence>
                  {showFilterPanel && (
                    ...
                    >
                      ...
                      >
                        <div>
                          <p className="text-xs font-semibold text-neutral-600 dark:text-neutral-400 text-left">
                            Active Filters
                          </p>
                      ...
                      {isFilterCardExpanded && (
                        <>
                          <div className="flex items-center justify-between mt-3 mb-2">
                            {draftFilterCount > 0 ? (
                              <div className="flex flex-wrap gap-2">
                                {draftSelectedEntries
                                  .slice(0, 2)
                                  .map(({ type, value }) => (
                                    <button
                                      key={`${type}-${value}`}
                                      onClick={() =>
                                        toggleDraftFilter(type, value)
                                      }
                ...
                >
                  <PieChart>
                    <Pie
                      ...
                      dataKey="value"
                      stroke="none"
                      onClick={(d) => toggleFilter("domain", d.fullName)}
                ...
                >
                  <PieChart>
                    <Pie
                      ...
                      dataKey="count"
                      stroke="none"
                      onClick={(d) => toggleFilter("category", d.name)}
                      className="cursor-pointer"
                    >
              ...
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={metrics.applicationData} barSize={25}>
                  ...
                  <Bar
                    dataKey="count"
                    radius={[6, 6, 0, 0]}
                    onClick={(d) => toggleFilter("application", d.name)}
                    className="cursor-pointer"
                  >
